...
# Versions are pinned to a build verified working on 2026-09-05. The three
# azure-* / isodate pins were added 2026-09-24, resolved against these pins
# from PyPI metadata (azure-core's requests dependency is apt's copy).
#
# Without pins pip resolves "newest on PyPI at build time", so an upstream
# release can break a build that changed nothing -- which is exactly how anyio
